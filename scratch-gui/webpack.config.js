...
// Add devServer proxy for OpenCode Zen API (CORS fix for local dev)
if (!buildDist && finalConfig.devServer) {
    var httpProxyMiddleware = require('http-proxy-middleware');
    var zenProxy = httpProxyMiddleware.createProxyMiddleware({
        target: 'https://opencode.ai',
        changeOrigin: true,
        secure: true
    });
    var existingSetup = finalConfig.devServer.setupMiddlewares;
    finalConfig.devServer.setupMiddlewares = function (middlewares, devServer) {
        devServer.app.use('/zen', zenProxy);
        if (existingSetup) return existingSetup(middlewares, devServer);
        return middlewares;
    };
}
